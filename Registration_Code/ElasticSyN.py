...
for i in mylist:
  ...
  mytx = ants.registration(fixed=fixed, moving=moving, type_of_transform='ElasticSyN' )
  mywarpedimage = ants.apply_transforms(fixed=fixed, moving=moving, transformlist=mytx['fwdtransforms'])
  mywarpedimage.to_file('/Output_ants_ElasticSyN/Registration/'+i)
  ...
  gm.to_file('/Output_ants_ElasticSyN/Greymatter/'+i)
  continue;
