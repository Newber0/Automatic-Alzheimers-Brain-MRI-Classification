...
datapath = ('/scratch/ex-rtam-1/original/MCIvsCN/')
...
for i in patients:
  ...
  if label == 'MCI':  # use `==` instead of `is` to compare strings
    labelset.append(0.)
  elif label == 'CN':
    labelset.append(1.)
  else:
...
plt.savefig('/scratch/ex-rtam-1/Baseline/BaselineMCIvsCNAcc.pdf')
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Test'], loc='upper left')
plt.savefig('/scratch/ex-rtam-1/Baseline/BaselineMCIvsCNLoss.pdf')
...
#model.save('/scratch/ex-rtam-1/Affine/AffineADvsCNModel.h5')