...
datapath = ('/arc/project/ex-rtam-1/Output_ants_SyNRA/DataSamples/ADvsCN/')
...
CNN_model = tf.keras.Sequential(
  [
      ...
      tf.keras.layers.Conv3D(kernel_size=(3, 3, 3), filters=64, activation='relu',
                          padding='same', strides=(2, 2, 2)),
      #tf.keras.layers.BatchNormalization(),
      tf.keras.layers.MaxPool3D(pool_size=(2, 2, 2), padding='same'),
      tf.keras.layers.Dropout(0.20),

      tf.keras.layers.Conv3D(kernel_size=(2, 2, 2), filters=128, activation='relu',
                          padding='same', strides=(2, 2, 2)),
      #tf.keras.layers.BatchNormalization(),
      tf.keras.layers.MaxPool3D(pool_size=(2, 2, 2), padding='same'),
      tf.keras.layers.Dropout(0.20), 
...
plt.savefig('/scratch/ex-rtam-1/SyNRA/Extra/SyNRAADvsCNAcc3layer.pdf')
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Test'], loc='upper left')
plt.savefig('/scratch/ex-rtam-1/SyNRA/Extra/SyNRAADvsCNLoss3layer.pdf')
...
#model.save('/scratch/ex-rtam-1/SyNRA/SyNRAADvsCNModel.h5')